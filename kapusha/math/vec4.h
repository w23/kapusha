...
template <typename T> struct vec4 {
  ...
  T length() const { return length2().sqrt(); }
  T rlength() const { return length2().rsqrt(); }
  vec4 recip() const { return vec4(x.recip(), y.recip(), z.recip(), w.recip()); }
  ...
  vec4<T>& operator*=(const vec4<T>& r) { return *this = *this + r; }
  vec4<T>& operator*=(const T &r) { return *this = *this + r; }
