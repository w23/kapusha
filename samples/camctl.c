...
static void paint(const KPwindow_painter_paint_t *paint) {
  ...
  KPdquatf q = kpDquatfMakeTransform(
    kpVec3fNormalize(kpVec3f(0, 1, 0)), pts, kpVec3f(0, 0, -5.f));
  KPmat4f m = kpMat4fMul(proj, kpMat4fMakeDquatf(kpDquatfMakeInverse(q)));
...
int kpuserAppCreate(int argc, const char *argv[]) {
  ...
  wp.title = "kapusha sample: textured camera";
  wp.user_data = 0;
